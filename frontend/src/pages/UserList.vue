  ...
  <div class="container mt-5">
    ...
    <div class="d-flex justify-content-center align-items-center mt-4">
      <button
        ...
        :key="page"
        :class="['btn', page === pagination.current_page ? 'btn-primary active' : 'btn-secondary']"
        :data-page="page"
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchUsers = async (page = 1) => {
      try {
        const response = await fetch(
          `http://localhost:8000/users/?age_min=${filters.value.ageMin}&age_max=${filters.value.ageMax}&page=${page}`,
          { credentials: "include" }
        );
    ...
    const applyFilters = () => {
      fetchUsers(1); // Reset to page 1 when applying filters
    };
